...
const BetLoading = ({ setBetDelay, betDelay,right }) => {
  useEffect(() => {
   if(betDelay > 0){
    setTimeout(() => {
  ...
  return (
    <div
      id="popup-modal"
      className="z-[1000] fixed  top-0 right-[0.5px] md:right-0 overflow-hidden flex w-full h-screen min-h-[100dvh] items-center justify-center bg-bg_CasinoPopupBg __web-inspector-hide-shortcut__"
    >
      <div className={`z-2 popUpBoxShadow popUpOpenAnimation absolute w-[86%] max-w-[380px] py-3 px-2 rounded-[10px] bg-bg_Quaternary p-2 xs:p-5 ${right ? 'right-10':''}`}>
        <div className="flex flex-col gap-1 align-top items-center">
          <span className="relative w-max min-w-6 min-h-6 flex items-center justify-center p-2 pt-6">
            ...
            <span className="font-semibold text-text_Ternary">{betDelay}</span>
          </span>
